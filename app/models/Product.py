class Product:
    def __init__(self, name, description, price, stock_quantity, category_id, image_url):
        self.name = name
    ...
    def add_product(self):
        ...
        VALUES (?, ?, ?, ?, ?, ?, GETDATE())
        """
        execute_query(query, (self.name, self.description, self.price, self.stock_quantity, self.category_id, self.image_url))
        print(f"تمت إضافة المنتج '{self.name}' بنجاح.")

    def update_product(self, product_id):
        query = """
        UPDATE Products
        SET Name = ?, Description = ?, Price = ?, StockQuantity = ?, CategoryID = ?, ImageURL = ?
        WHERE ProductID = ?
        """
